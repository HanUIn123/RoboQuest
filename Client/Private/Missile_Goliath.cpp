...
CMissile_Goliath::CMissile_Goliath(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    :CMissile{_pDevice, _pContext}
{
}

CMissile_Goliath::CMissile_Goliath(const CMissile_Goliath& _Prototype)
    :CMissile(_Prototype)
{
}

HRESULT CMissile_Goliath::Initialize_Prototype()
{
...
HRESULT CMissile_Goliath::Initialize(void* _pArg)
{
    MISSILE_GOLIATH_DESC* pDesc = static_cast<MISSILE_GOLIATH_DESC*>(_pArg);
...
void CMissile_Goliath::Priority_Update(_float _fTimeDelta)
{
}

void CMissile_Goliath::Update(_float _fTimeDelta)
{
    if (!m_bActive)
    {
        return;
...
void CMissile_Goliath::Late_Update(_float _fTimeDelta)
{
...
HRESULT CMissile_Goliath::Render()
{
...
void CMissile_Goliath::Check_Collision_With_Player()
{
...
HRESULT CMissile_Goliath::Ready_Component()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Missile_Goliath"), TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
        return E_FAIL;
...
HRESULT CMissile_Goliath::Bind_ShaderResource()
{
    // 월드 행렬 바인딩. 
...
void CMissile_Goliath::Activate_Missile(const _vector& _vPosition, const _vector& _vGunMuzzleLook)
{
...
//void CMissile_Goliath::Activate_MortalMissile(const _vector& _vPosition, const _vector& _vGunMuzzleLook)
//{
//    __super::Activate_Missile();
//
//    m_vInitialMortalMissilePos = _vPosition;
//    m_pTransformCom->Set_State(CTransform::STATE_POSITION, _vPosition);
//    m_pTransformCom->LookAt(_vPosition + _vGunMuzzleLook);
//}

void CMissile_Goliath::DeActivate_Missile()
{
    m_bActive = false;
...
void CMissile_Goliath::Missile_Fly(_float _fTimeDelta)
{
    if (!m_bActive)
        return;
...
CMissile_Goliath* CMissile_Goliath::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CMissile_Goliath* pInstance = new CMissile_Goliath(_pDevice, _pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Created: CMissile_Goliath");
...
CGameObject* CMissile_Goliath::Clone(void* _pArg)
{
    CMissile_Goliath* pInstance = new CMissile_Goliath(*this);
