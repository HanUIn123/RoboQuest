...
CBullet_Sniper::CBullet_Sniper(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    :CBullet(_pDevice, _pContext)
{
}

CBullet_Sniper::CBullet_Sniper(const CBullet& _Prototype)
    :CBullet(_Prototype)
{
}

HRESULT CBullet_Sniper::Initialize_Prototype()
{
...
HRESULT CBullet_Sniper::Initialize(void* _pArg)
{
    BULLET_SNIPER_DESC* pDesc = static_cast<BULLET_SNIPER_DESC*>(_pArg);

    pDesc->fSpeedPerSec = 130.f;
...
void CBullet_Sniper::Priority_Update(_float _fTimeDelta)
{
    if (!m_bActive)
    {
...
void CBullet_Sniper::Update(_float _fTimeDelta)
{
...
void CBullet_Sniper::Late_Update(_float _fTimeDelta)
{
...
HRESULT CBullet_Sniper::Ready_Component()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_SniperBullet"), TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
        return E_FAIL;
    ...
    if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_SniperTrailTexture"), TEXT("Com_Texture"), reinterpret_cast<CComponent**>(&m_pTextureCom))))
        return E_FAIL;

    return S_OK;
}

HRESULT CBullet_Sniper::Bind_ShaderResource()
{
    // 월드 행렬 바인딩. 
...
void CBullet_Sniper::Activate_Bullet(const _vector& _vPosition, const _vector& _vGunMuzzleLook)
{
...
void CBullet_Sniper::Set_Bullet_To_Muzzle(const _vector& _vGunMuzzlePos, const _vector& _vGunMuzzleLook)
{
...
void CBullet_Sniper::Bullet_Fly(_float _fTimeDelta)
{
...
void CBullet_Sniper::DeActivate_Bullet()
{
...
void CBullet_Sniper::Set_RayDistance(_float _fRayDistance)
{
    __super::Set_RayDistance(_fRayDistance);
}

void CBullet_Sniper::Initialize_Ray()
{
    __super::Initialize_Ray();
}

CEffect_Impact* CBullet_Sniper::Find_Inactive_ImpactEffect()
{
...
CBullet_Sniper* CBullet_Sniper::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CBullet_Sniper* pInstance = new CBullet_Sniper(_pDevice, _pContext);
    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Created: CBullet_Sniper");
...
CGameObject* CBullet_Sniper::Clone(void* _pArg)
{
