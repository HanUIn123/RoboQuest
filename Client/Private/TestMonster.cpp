...
CTestMonster::CTestMonster(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    :CMapToolMonster{ _pDevice, _pContext }
{

}

CTestMonster::CTestMonster(const CTestMonster& _Prototype)
...
HRESULT CTestMonster::Initialize(void* _pArg)
{
    if (nullptr != _pArg)
    {
        TESTMONSTER_DESC* pDesc = static_cast<TESTMONSTER_DESC*>(_pArg);
...
void CTestMonster::Priority_Update(_float _fTimeDelta)
{

}

void CTestMonster::Update(_float _fTimeDelta)
{
...
void CTestMonster::Late_Update(_float _fTimeDelta)
{
...
HRESULT CTestMonster::Ready_Component()
{
    ...
    if (FAILED(__super::Add_Component(LEVEL_MAPTOOL, TEXT("Prototype_Component_Model_BillyBoom"), TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModelCom))))
        return E_FAIL;
...
HRESULT CTestMonster::Bind_ShaderResource()
{
    // 월드 행렬 바인딩. 
...
CTestMonster* CTestMonster::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CTestMonster* pInstance = new CTestMonster(_pDevice, _pContext);
...
CGameObject* CTestMonster::Clone(void* _pArg)
{
