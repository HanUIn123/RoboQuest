...
BEGIN(Client)
class CPlayer;
class CBullet_HandGun final : public CBullet
{
public:
    struct BULLET_HANDGUN_DESC : public CGameObject::GAMEOBJECT_DESC
    {

    };
private:
    CBullet_HandGun(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
    CBullet_HandGun(const CBullet& _Prototype);
    virtual ~CBullet_HandGun() = default;
...
private:
    ...
    //vector<_float3>                     m_vecBulletPosition;
...
public:
    static CBullet_HandGun*             Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
