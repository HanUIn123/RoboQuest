...
BEGIN(Client)
class CPlayer;
class CRifle final : public CGun
{
public:
	enum RIFLE_STATE {
		...
		STATE_RELOAD = 0x00000001,
		STATE_FIRE = 0x00000002
	};

public:
	struct RIFLE_DESC : public CPartObject::PARTOBJECT_DESC
	{
		_uint				iPistolState = {};
...
public:
	_vector CRifle::Get_MuzzlePosition() const {
...
private:
	CRifle(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
	CRifle(const CGun& _Prototype);
