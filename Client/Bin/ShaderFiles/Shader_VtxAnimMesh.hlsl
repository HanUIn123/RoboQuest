...
bool g_EffectiveObject;

Texture2D g_MaskTexture;

float g_Timer;

float           g_RimPower = 0.1f;
...
PS_OUT PS_MAIN(PS_IN In)
{
    ...
    if(g_EffectiveObject)
    {
        float fDissolveValue = g_DissolveTexture.Sample(LinearSampler, In.vTexcoord);

        if (fDissolveValue < g_DissolveValue)
        {
            float Result = smoothstep(0.0f, 1.0f, g_DissolveValue * 0.5f);
            vMtrlDiffuse.rgb = lerp(float3(1.0, 0.3, 0.0), vMtrlDiffuse.rgb, Result);
        }
...
VS_OUT_GLOW VS_MAIN_GLOW(VS_IN In)
{
...
PS_OUT_GLOW PS_MAIN_GLOW(PS_IN In)
{
    ...
    vector vDiffuseMaterial = g_DiffuseTexture.Sample(LinearSampler, In.vTexcoord);
    
    //vector vMaterialMask = g_MaskTexture.Sample(LinearSampler, In.vTexcoord);
    //
    //float3 lavaColor = lerp(float3(1.0, 0.3, 0.0), float3(1.0, 0.8, 0.0), vMaterialMask.r);
    
    //Out.vColor = vector(1.0f, 1.0f, 1.0f, 1.0f);
    //Out.vColor = vector(lavaColor, vMaterialMask.a);
    //Out.vColor = vector(lavaColor, 1.0f);
    
    
    float3 LarvaColor = lerp(float3(1.0, 0.3, 0.0), float3(1.0, 0.8, 0.0), vDiffuseMaterial.r);
    Out.vColor = vector(LarvaColor, 1.0f);
...
PS_OUT_RIMLIGHT PS_RIMLIGHT(PS_IN In)
{
...
struct PS_OUT_RIMDISCARD
{
    float4 vColor : SV_TARGET0;
};

PS_OUT_RIMDISCARD PS_RimLightDiscard(PS_IN In)
{
    PS_OUT_RIMDISCARD Out = (PS_OUT_RIMDISCARD) 0;
    
    float4 texColor = g_DiffuseTexture.Sample(LinearSampler, In.vTexcoord + g_Timer * float2(0.1f, 0.2f));
    
    float3 LarvaColor = lerp(float3(1.0f, 0.3f, 0.0f), float3(1.0f, 0.8f, 0.0f), texColor.r);
...
technique11 DefaultTechnique
{
    ...
    pass Glow
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN_GLOW();
        GeometryShader = NULL;
    ...
    pass RimLightDiscard
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_RimLightDiscard();
    }
}