...
HRESULT CGraphic_Device::Ready_SwapChain(HWND _hWnd, _bool _isWindowed, _uint _iWinCX, _uint _iWinCY)
{
	...
	/* 만든 픽셀하나의 데이터 정보 : 32BIT픽셀생성하되 부호가 없는 정규화된 수를 저장할께 */
	SwapChain.BufferDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM; 	
	// 따로 명시해 두지 않고, scaling작업을 함께 하겠다,(백버퍼에 맞춰서 진행한다) 
	...
	// 본인 컴이 144주사율이면 144로 해도 된단다.
	SwapChain.BufferDesc.RefreshRate.Numerator = 60;
	SwapChain.BufferDesc.RefreshRate.Denominator = 1;

	/* 멀티샘플링 : 안티얼라이징 (계단현상방지) */ // 픽셀이 각져 곱창나는 걸, 뭉개서 섞는 식으로 한다는 의미.
	/* 나나중에 후처리 렌더링 : 멀티샘플링 지원(x) */ 
	// 나중에 진행할 후처리 셰이딩에 오류발생할 가능성이 있어서, 이 안티얼라이징 작업은 하지않는다.
...
HRESULT CGraphic_Device::Ready_DepthStencilView(_uint _iWinCX, _uint _iWinCY)
{
	...
	// 텍스쳐 픽셀의 초기값 : nullptr 원래는 black인데 clear로 계속 호출 하기에 blue로 보임.
