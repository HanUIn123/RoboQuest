...
CSkyBoxCube::CSkyBoxCube(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    :CVIBuffer{ _pDevice, _pContext }
{
}

CSkyBoxCube::CSkyBoxCube(const CSkyBoxCube& _Prototype)
    :CVIBuffer(_Prototype)
{
}

HRESULT CSkyBoxCube::Initialize_Prototype()
{
	m_iNumVertices = 8;
...
HRESULT CSkyBoxCube::Initialize(void* _pArg)
{
    return S_OK;
}

CSkyBoxCube* CSkyBoxCube::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
    CSkyBoxCube* pInstance = new CSkyBoxCube(_pDevice, _pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Created : CSkyBoxCube");
...
CComponent* CSkyBoxCube::Clone(void* _pArg)
{
    CSkyBoxCube* pInstance = new CSkyBoxCube(*this);
